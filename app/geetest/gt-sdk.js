...
Geetest.prototype = {
  ...
  async validate(fallback, result, callback) {
    const that = this;
    return new Promise(function(resolve, reject) {
  ...
  _validate(fallback, result, callback) {
    const challenge = result.challenge || result.geetest_challenge;
    const validate = result.validate || result.geetest_validate;
    const seccode = result.seccode || result.geetest_seccode;
    if (fallback) {
  ...
  _make_challenge() {
    ...
    const rnd2 = randint(0, 90);
    const md5_str1 = md5(rnd1);
